...
def get_vivado_build_targets(data):
    ...
    for design in data['designs']:
        # Note: the old per-family HLS token ('arch') and the dead
        # 'hls_solution' field are intentionally NOT emitted -- the IP is now
        # built per-target for the target's real device, and build.tcl points
        # ip_repo_paths at HLS/<core>/<target>/.
        target = 'dict set target_dict {} {{ {} {} {} {} {} }}'.format(
            design['label'],design['url'],design['boardname'],
            design['bdscript'],design['sim_bd'],design['lanecfg'])
